    ...
    void observe_metrics(drive_iocb* iocb) {
        ...
        case DriveOpType::WRITE:
         ...
         //   LOGTRACE("write, size {}, lat {}", iocb->size, dur);
			...
			if(wcount % 10000 == 0) {
				LOGINFO("write, total count {} average size {}, average lat {}",wcount, wsize/ 10000 , wdur/ 10000);
				wdur = 0;
				wsize = 0;
			}
            break;
        case DriveOpType::READ:
          ...
          // LOGTRACE("read, size {}, lat {}", iocb->size, dur);
            // HISTOGRAM_OBSERVE(*m_metrics, read_lat, dur);
            // HISTOGRAM_OBSERVE(*m_metrics, read_size, iocb->size);
           LOGINFO("single read, size {}, lat {}", iocb->size, dur);
           if (count % 10000 == 0) LOGINFO("read, size {}, lat {}", iocb->size, dur);
			rcount++;
			rdur += dur;
			rsize += iocb->size;
			if(rcount % 10000 == 0) {
				LOGINFO("read, total count {} average size {}, average lat {}",rcount, rsize/ 10000 , rdur/ 10000);
				rdur = 0;
				rsize = 0;
			}
            break;
        case DriveOpType::FSYNC:
          ...
          //  LOGTRACE("fsync, size {}, lat {}", iocb->size, dur);
            // HISTOGRAM_OBSERVE(*m_metrics, fsync_lat, dur);
            // HISTOGRAM_OBSERVE(*m_metrics, fsync_size, iocb->size);
            if (count % 10000 == 0) LOGINFO("fsync, size {}, lat {}", iocb->size, dur);
			fcount++;
			fdur += dur;
			fsize += iocb->size;
			if(fcount % 10000 == 0) {
				LOGINFO("fsync, total count {} average size {}, average lat {}", fcount, fsize/ 10000 , fdur/ 10000);
				fdur = 0;
				fsize = 0;
			}
            break;
